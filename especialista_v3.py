...
import numpy as np
import sys
if 'test_conexion' in sys.modules:
    exchange = sys.modules['test_conexion'].exchange
else:
    from test_conexion import exchange

# ── Historial BTC para suavizado (persiste entre llamadas) ──────────────────
_btc_slopes_hist = []
_BTC_SUAVIZADO_N = 5   # media de 5 scans (~15s) elimina el ruido
...
def analizar_sniper(simbolo, k_lim=45, ma_tipo="SMA"):
    ...
    res = {
        ...
        'obv_slope': 0.0,
        # Contexto BTC — mejorado
        'btc_precio': 0, 'btc_slope': 0,
        'btc_slope_suav': 0,   # slope suavizado N scans — sin ruido
        'btc_1m_slope': 0,     # LOG: momentum inmediato
        'btc_5m_slope': 0,     # LOG: impulso corto
        'btc_trend': '',
        'btc_trend_suav': '',  # trend sin ruido — usado en filtros
        'btc_1m_trend': '',    # LOG
        'btc_5m_trend': '',    # LOG
        'btc_score': 0,        # LOG: score multi-TF de -3 a +3
        'btc_retorno_1h': 0,   # LOG: retorno BTC en última hora
        'corr_btc': 0,         # LOG: correlación símbolo vs BTC
        'divergencia_btc': 0,  # LOG: slope_símbolo - slope_btc_5m
        'block': '', 'error': False
    }
    try:
        # Fetch con timeout real usando threading
        import threading

        def fetch_con_timeout(func, args, kwargs, timeout_s=8):
            resultado = [None]
            error = [None]
            def worker():
                try:
            ...
            t.join(timeout_s)
            if t.is_alive():
                raise TimeoutError(f"fetch timeout despues de {timeout_s}s")
            if error[0]:
                raise error[0]
            return resultado[0]

        # Fetch de velas (1h)
        velas = fetch_con_timeout(exchange.fetch_ohlcv, [simbolo], {'timeframe': '1h', 'limit': 210})
        ...
        c15 = pd.Series(dtype=float)  # para correlación BTC
        try:
            # params='future' fuerza el mercado correcto en conexiones de futuros
            velas_15m = fetch_con_timeout(exchange.fetch_ohlcv, [simbolo], {'timeframe': '15m', 'limit': 100, 'params': {'price': 'mark'}})
            ...
            pass  # error 15m silenciado

        # --- CONTEXTO BTC — MEJORADO ---
        # Problema anterior: umbral ±0.02% en 15m → cambiaba cada 1.3min (83% ruido)
        # Solución: umbral ±0.05% + slope suavizado N scans + slopes 1m y 5m
        global _btc_slopes_hist

        btc_precio = btc_slope = btc_slope_suav = 0.0
        btc_trend = btc_trend_suav = ''
        btc_1m_slope = btc_5m_slope = 0.0
        btc_1m_trend = btc_5m_trend = ''
        btc_score = 0
        btc_retorno_1h = corr_btc = 0.0

        try:
            if simbolo != 'BTC/USDT:USDT':
                # 15m — base
                velas_btc = fetch_con_timeout(
                    exchange.fetch_ohlcv, ['BTC/USDT:USDT'],
                    {'timeframe': '15m', 'limit': 24}
                )
                if not velas_btc or len(velas_btc) < 5:
                    raise ValueError(f"Datos BTC insuficientes: {len(velas_btc) if velas_btc else 0}")
                ...
                btc_slope  = round(((df_btc['ema7'].iloc[-1] - df_btc['ema7'].iloc[-2]) / df_btc['ema7'].iloc[-2]) * 100, 4)

                # Suavizado: acumula últimos N slopes
                _btc_slopes_hist.append(btc_slope)
                if len(_btc_slopes_hist) > _BTC_SUAVIZADO_N:
                    _btc_slopes_hist.pop(0)
                btc_slope_suav = round(sum(_btc_slopes_hist) / len(_btc_slopes_hist), 4)

                # Umbral ampliado a ±0.05% (antes ±0.02%)
                UMBRAL = 0.05
                btc_trend      = 'ALCISTA' if btc_slope      >  UMBRAL else ('BAJISTA' if btc_slope      < -UMBRAL else 'LATERAL')
                btc_trend_suav = 'ALCISTA' if btc_slope_suav >  UMBRAL else ('BAJISTA' if btc_slope_suav < -UMBRAL else 'LATERAL')

                # Retorno BTC última hora (4 velas de 15m)
                if len(c_btc) >= 5:
                    btc_retorno_1h = round(((c_btc.iloc[-1] - c_btc.iloc[-5]) / c_btc.iloc[-5]) * 100, 4)

                # Correlación símbolo vs BTC (últimas 20 velas 15m)
                if len(c_btc) >= 20 and len(c15) >= 20:
                    ret_btc = c_btc.pct_change().dropna().tail(19).values
                    ret_sim = c15.pct_change().dropna().tail(19).values
                    if len(ret_btc) == len(ret_sim) and ret_btc.std() > 0 and ret_sim.std() > 0:
                        corr_btc = round(float(np.corrcoef(ret_btc, ret_sim)[0, 1]), 3)

                # BTC 1m — momentum inmediato
                try:
                    velas_1m = fetch_con_timeout(
                        exchange.fetch_ohlcv, ['BTC/USDT:USDT'],
                        {'timeframe': '1m', 'limit': 10}
                    )
                    if velas_1m and len(velas_1m) >= 3:
                        df_1m   = pd.DataFrame(velas_1m, columns=['t','o','h','l','c','v'])
                        ema7_1m = df_1m['c'].astype(float).ewm(span=7, adjust=False).mean()
                        btc_1m_slope = round(((ema7_1m.iloc[-1] - ema7_1m.iloc[-2]) / ema7_1m.iloc[-2]) * 100, 4)
                        btc_1m_trend = 'ALCISTA' if btc_1m_slope > 0.01 else ('BAJISTA' if btc_1m_slope < -0.01 else 'LATERAL')
                except Exception:
                    pass

                # BTC 5m — impulso corto
                try:
                    velas_5m = fetch_con_timeout(
                        exchange.fetch_ohlcv, ['BTC/USDT:USDT'],
                        {'timeframe': '5m', 'limit': 10}
                    )
                    if velas_5m and len(velas_5m) >= 3:
                        df_5m   = pd.DataFrame(velas_5m, columns=['t','o','h','l','c','v'])
                        ema7_5m = df_5m['c'].astype(float).ewm(span=7, adjust=False).mean()
                        btc_5m_slope = round(((ema7_5m.iloc[-1] - ema7_5m.iloc[-2]) / ema7_5m.iloc[-2]) * 100, 4)
                        btc_5m_trend = 'ALCISTA' if btc_5m_slope > 0.03 else ('BAJISTA' if btc_5m_slope < -0.03 else 'LATERAL')
                except Exception:
                    pass

                # Score multi-TF de -3 a +3 (LOG — no filtra todavía)
                s1  = 1 if btc_1m_slope  >  0.01 else (-1 if btc_1m_slope  < -0.01 else 0)
                s5  = 1 if btc_5m_slope  >  0.03 else (-1 if btc_5m_slope  < -0.03 else 0)
                s15 = 1 if btc_slope_suav >  0.05 else (-1 if btc_slope_suav < -0.05 else 0)
                btc_score = s1 + s5 + s15

        except Exception as ebtc:
            open('error_esp.txt', 'a').write(f"[ERROR_BTC]: {ebtc}\n")
        ...
        res.update({
            ...
            'btc_slope':       btc_slope,
            'btc_slope_suav':  btc_slope_suav,
            'btc_1m_slope':    btc_1m_slope,
            'btc_5m_slope':    btc_5m_slope,
            'btc_trend':       btc_trend,
            'btc_trend_suav':  btc_trend_suav,
            'btc_1m_trend':    btc_1m_trend,
            'btc_5m_trend':    btc_5m_trend,
            'btc_score':       btc_score,
            'btc_retorno_1h':  btc_retorno_1h,
            'corr_btc':        corr_btc,
            'divergencia_btc': round(p7 - btc_5m_slope, 4),
            # ATR multi-timeframe
            'atr_1h':  atr_1h,
            'atr_15m': atr_15m,
            'atr_5m':  atr_5m,
        ...
        res['error'] = True
        open('error_esp.txt','w').write(str(e))
